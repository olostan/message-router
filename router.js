...
function RouterStart() {
    ...
    handlerScripts.forEach(function (handlerFile) {
        ...
        balancer.onNewWorker(function(worker) {
            worker.worker.send({
                cmd:'$workflow',
                workflow: workflow
            });
        ...
        toRegister.forEach(function(r) { routingTable[r] = balancer })
    });
}
function addHanldersDir(dir) {
    var handers = fs.readdirSync(dir);
    handers.forEach(handlerScripts.push);
}

function ImportConfig(cfg) {
    for(var c in cfg) {
        if (!cfg.hasOwnProperty(r)) continue;
        config[c] = cfg[c];
    }
}
